 ...
 * Formule GDD §5 : 3000ms ÷ (Speed ÷ 100)
 * Cap minimum 800ms (Pokémon bas niveau) / maximum 5000ms
...
export function calcActionDelay(pokemon: CombatPokemon): number {
  ...
  const delay = Math.round(3000 / (speed / 100))
  return Math.max(800, Math.min(delay, 5000))
}
